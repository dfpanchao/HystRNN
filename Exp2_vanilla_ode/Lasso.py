...
clf = linear_model.Lasso(fit_intercept=True, alpha=0.1).fit(X, y)
...
## Obtained ODE
## B'' = 192.6 B + 24.2 B' -761.1 H -18.9 |B|^2 -1.2 |B'|^2 -109.1 |H|^2 + 54.8
...
def system(state, t, h_interp):
    ...
    h_val = h_interp(t)
    dx1_dt = x2
    dx2_dt = (192.6 * x1 + 24.2 * x2 - 761.1 * h_val - 18.9 * np.abs(x1)**2 - 1.2 * np.abs(x2)**2 - 109.1 * np.abs(h_val)**2 + 54.8)
...
plt.savefig('Figures/ode_lasso_1.jpeg', dpi=500, bbox_inches="tight")
plt.show()
...
plt.savefig('Figures/ode_lasso_2.jpeg', dpi=500, bbox_inches="tight")
plt.show()
...
plt.savefig('Figures/ode_lasso_3.jpeg', dpi=500, bbox_inches="tight")
plt.show()
...
plt.savefig('Figures/ode_lasso_4.jpeg', dpi=500, bbox_inches="tight")
plt.show()

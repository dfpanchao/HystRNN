...
size_N = 300 #300, 600, 900, 1200
size_M1 = 100 #100, 200, 300, 400
size_M2 = 2*size_M1
...
B_major = B_Major_FORC[:595] # Needs N
H_major = H_Major_FORC[:595] # Needs N

original_length = B_major.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_N) ########
interpolator = interp1d(x_original, B_major.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
B_major = interpolated_array.reshape(-1, 1) ########

original_length = H_major.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_N) ########
interpolator = interp1d(x_original, H_major.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
H_major = interpolated_array.reshape(-1, 1) ########

B1 = B_Major_FORC[596:795] # Needs M1
H1 = H_Major_FORC[596:795] # Needs M1

original_length = B1.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M1) ########
interpolator = interp1d(x_original, B1.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
B1 = interpolated_array.reshape(-1, 1) ########

original_length = H1.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M1) ########
interpolator = interp1d(x_original, H1.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
H1 = interpolated_array.reshape(-1, 1) ########

B2 = B_Major_FORC[995:] # Needs M1
H2 = H_Major_FORC[995:] # Needs M1

original_length = B2.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M1) ########
interpolator = interp1d(x_original, B2.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
B2 = interpolated_array.reshape(-1, 1) ########

original_length = H2.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M1) ########
interpolator = interp1d(x_original, H2.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
H2 = interpolated_array.reshape(-1, 1) ########
...
B_amp1 = loaded_data_B_amp1.reshape(-1,1) # Needs M2
H_amp1 = loaded_data_H_amp1.reshape(-1,1) # Needs M2
B_amp2 = loaded_data_B_amp2.reshape(-1,1) # Needs M2
H_amp2 = loaded_data_H_amp2.reshape(-1,1) # Needs M2

original_length = B_amp1.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M2) ########
interpolator = interp1d(x_original, B_amp1.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
B_amp1 = interpolated_array.reshape(-1, 1) ########

original_length = H_amp1.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M2) ########
interpolator = interp1d(x_original, H_amp1.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
H_amp1 = interpolated_array.reshape(-1, 1) ########

original_length = B_amp2.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M2) ########
interpolator = interp1d(x_original, B_amp2.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
B_amp2 = interpolated_array.reshape(-1, 1) ########

original_length = H_amp2.shape[0]  ########
x_original = np.arange(original_length)
x_new = np.linspace(0, original_length - 1, size_M2) ########
interpolator = interp1d(x_original, H_amp2.flatten(), kind='cubic')  ########
interpolated_array = interpolator(x_new)
H_amp2 = interpolated_array.reshape(-1, 1) ########
...
B_major_scaled_train = B_Major_FORC_scaled[:595] # Needs N
H_major_scaled_train = H_Major_FORC_scaled[:595] # Needs N

B1_scaled_test = B_Major_FORC_scaled[596:795] # Needs M1
H1_scaled_test = H_Major_FORC_scaled[596:795] # Needs M1

B2_scaled_test = B_Major_FORC_scaled[995:] # Needs M1
H2_scaled_test = H_Major_FORC_scaled[995:] # Needs M1

B_amp1_scaled = (loaded_data_B_amp1 - B_min) / (B_max - B_min) * 2 - 1
H_amp1_scaled = (loaded_data_H_amp1 - H_min) / (H_max - H_min) * 2 - 1

B_amp1_scaled_test = B_amp1_scaled.reshape(-1,1) # Needs M2
H_amp1_scaled_test = H_amp1_scaled.reshape(-1,1) # Needs M2

B_amp2_scaled = (loaded_data_B_amp2 - B_min) / (B_max - B_min) * 2 - 1
H_amp2_scaled = (loaded_data_H_amp2 - H_min) / (H_max - H_min) * 2 - 1

B_amp2_scaled_test = B_amp2_scaled.reshape(-1,1) # Needs M2
H_amp2_scaled_test = H_amp2_scaled.reshape(-1,1) # Needs M2
...
# ax.plot(H1[2:], test_output_data_1, color='black', linestyle='dashdot', linewidth=3, label='pred')
#
...
# # axins = inset_axes(ax, width="40%", height="80%", loc='upper left')
# #
...
# # axins.tick_params(axis='both', which='major', labelsize=10, width=1, length=4)
# #
...
#
# plt.savefig('Figures/a3_HystRNN.jpeg', dpi=500, bbox_inches="tight")
# plt.show()
#
# #####################################################
# ################# Fig. 3d ###########################
...
# # axins.set_yticks([-0.75, 0.0, 0.75])
# #
# # # Set tick labels fontweight to bold and increase font size for the inset axis
# # axins.tick_params(axis='both', which='major', labelsize=10, width=1, length=4)
# #
...
# # axins.spines['left'].set_linewidth(1)
#
# plt.savefig('Figures/d3_HystRNN.jpeg', dpi=500, bbox_inches="tight")
# plt.show()
#
# #####################################################
# ################# Fig. 3g ###########################
...
# ax.plot(H_amp1[2:], test_output_data_amp1, color='black', linestyle='dashdot', linewidth=3, label='pred')
#
#
# # Set the axis labels with bold font weight
# ax.set_xlabel(r"H[A/m]", fontsize=28, color='black')
...
# # axins.spines['left'].set_linewidth(1)
#
# plt.savefig('Figures/g3_HystRNN.jpeg', dpi=500, bbox_inches="tight")
# plt.show()
#
# #####################################################
# ################# Fig. 3j ###########################
...
# ax.plot(H_amp2[2:], test_output_data_amp2, color='black', linestyle='dashdot', linewidth=3, label='pred')
#
#
# # Set the axis labels with bold font weight
# ax.set_xlabel(r"H[A/m]", fontsize=28, color='black')
...
# # axins.spines['left'].set_linewidth(1)
#
# plt.savefig('Figures/j3_HystRNN.jpeg', dpi=500, bbox_inches="tight")
# plt.show()
